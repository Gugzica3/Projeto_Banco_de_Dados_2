...
- **Função**: Roda em **Python 3** para gerar dados fictícios e simular requisições.
- **Responsabilidades**:
  - Criar 10 usuários
  - Criar 20 jogos no catálogo
  - Simular 50 pedidos de amizade
  - Simular 100 compras de jogos
  - Salvar log de verificação local (`s1_verification_log.json`)
...
- **Porta**: 3003
- **Banco**: Neo4j
- **Responsabilidade**: Gerencia a rede social e recomendações (stubs).
...
O **PostgreSQL** foi escolhido para armazenar dados de **Usuários** e **Bibliotecas** due às seguintes características críticas:
...
**c) Dados Estruturados e Previsíveis**
  ...
  - Esquema de autenticação (email, senha\_hash) é fixo e bem definido
...
O **Neo4j** foi escolhido para **Rede Social** e **Sistema de Recomendações** devido à natureza highly conectada dos dados:
...
| GET | `/users/:id/recommendations` | Obter recomendações (stub) |
...
### Pré-requisitos
  ...
  - **Python 3.10+** (para executar S1 localmente)
...
```bash
git clone [https://github.com/seu-usuario/mini-steam.git](https://github.com/seu-usuario/mini-steam.git)
cd mini-steam
```

#### 2\. Limpar Testes Anteriores (Recomendado)

Para garantir que os bancos de dados estejam vazios e os scripts de inicialização rodem corretamente, execute:

```bash
docker-compose down -v
```

#### 3\. Iniciar os Bancos de Dados e Microsserviços (S2)
...
**Aguarde \~30 segundos** para que todos os serviços estejam prontos e os healthchecks passem.

#### 4\. Executar o Simulador S1 (Python)

**Abra um novo terminal** e navegue até a pasta do S1 para configurar o ambiente Python.

```bash
# Entre na pasta do S1
cd servico-s1
...
# Instale as dependências (requests, etc.)
...
6.  ✅ Salvar `s1_verification_log.json`

#### 5\. Verificar Resultados

```bash
# Ver log de verificação gerado
...
### ⚠️ Solução de Problemas: Erros "409 Conflict"

Se você rodar o `simulator.py` uma segunda vez **sem limpar os bancos**, você verá erros `409 Conflict` no seu log.

**Isso não é um bug\!** É o seu sistema funcionando corretamente. Significa que os dados (ex: "João Silva" ou "The Witcher 3") já existem, e seus microsserviços estão (corretamente) impedindo a criação de duplicatas.

Para rodar o simulador em um banco de dados limpo, use o comando de limpeza total:
...
```bash
# Parar todos os containers (sem apagar dados)
docker-compose down
```
...
│       └── 01-init-db.js
│
├── servico-usuarios/            # Microsserviço 1 (Node.js + PostgreSQL)
│   ├── Dockerfile
│   ├── package.json
│   ├── server.js
│   └── .env
│
├── servico-catalogo/            # Microsserviço 2 (Node.js + MongoDB)
│   ├── Dockerfile
│   ├── package.json
│   ├── server.js
│   └── .env
│
├── servico-social/              # Microsserviço 3 (Node.js + Neo4j)
│   ├── Dockerfile
│   ├── package.json
│   ├── server.js
│   └── .env
│
└── servico-s1/                  # Cliente Simulador (Python)
    ├── requirements.txt         # Dependências Python
    ├── simulator.py             # Script de simulação
...
// Ver compras
MATCH path = (:User)-[:OWNS]->(:Game)
RETURN path
LIMIT 50
```

### Arquivo de Verificação (s1\_verification\_log.json)

Após executar S1, o arquivo conterá um resumo de todas as operações:
